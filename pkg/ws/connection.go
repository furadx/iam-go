...
const (
	...
	// defaultPingPeriod 默认 ping 间隔，需小于客户端读超时（60s）
	defaultPingPeriod = 30 * time.Second
)

// Connection 表示一个 WebSocket 连接的封装。
// 实现单写者模式：readLoop 读取消息，writeLoop 唯一写者发送消息和 ping。
...
func (conn *Connection) readLoop() {
	defer func() {
		if r := recover(); r != nil {
			log.Error("ws readLoop panic recovered",
				zap.Any("panic", r),
...
func (conn *Connection) writeLoop() {
	defer func() {
		if r := recover(); r != nil {
			log.Error("ws writeLoop panic recovered",
				zap.Any("panic", r),
				zap.String("stack", string(debug.Stack())))
		}
		conn.Close()
